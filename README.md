...
```
docker run --env GETH_CONFIG_DIR=/tmp/config.toml --env GENESIS_INIT_PATH=https://raw.githubusercontent.com/parithosh/consensus-deployment-ansible/master/kintsugi-testnet/custom_config_data/genesis.json --env CONFIG-Eth-NetworkId=1337702 0labs/geth:latest geth --config /tmp/config.toml --catalyst 
```
